...
__all__ = [
    'Module', 'Conv2d', 'Linear', 'BatchNorm2d', 'Sequential', 'get_child_dict'
]
...
def get_child_dict(params, key=None):
  ...
  """
    ...
    child_dict = OrderedDict((key_re.sub(r'\1', k), value)
        ...
        if self.track_running_stats:
            if self.episodic:
                for ep in range(n_episode):
                    self.register_buffer('running_mean_%d' % ep,
                                         torch.zeros(num_features))
                    self.register_buffer('running_var_%d' % ep,
                                         torch.ones(num_features))
                    self.register_buffer('num_batches_tracked_%d' % ep,
        ...
        if self.track_running_stats:
            ...
            if self.training:
                ...
                if self.alpha:
                    ...
                    alpha = torch.sigmoid(self.alpha_scale * self.batch_size +
                                          self.alpha_offset)
                    ...
                    return F.batch_norm(x, running_mean, running_var, weight,
                                        bias, False, 0., self.eps)
        else:
            return F.batch_norm(x, None, None, weight, bias, True, 0.,
                                self.eps)
